...
# Streamlit page setup with color
st.set_page_config(page_title="I Can Retrieve Any Data", page_icon="🔍", layout="wide")
st.markdown("<h1 style='color: #FF5733;'>TeliCo Query Form</h1>", unsafe_allow_html=True)

# Get user input
question = st.text_input("Input your question:", key="input", help="Ask your question related to telecom data.", 
                         label_visibility="visible", placeholder="e.g., Show me customers with high GB usage", max_chars=150)

# Submit button with custom color
submit = st.button("Ask the question", key="submit", help="Click to submit your question",
                   use_container_width=True, style="background-color: #FF5733; color: white; font-size: 16px; font-weight: bold;")
        ...
        if generic_response:
            ...
            if sql_query:
                try:
                    ...
                    # Display Results with color styling
                    st.subheader("Query Results:")
                    if results:
                        for row in results:
                            st.write(f"<div style='color: #28A745;'>{row}</div>", unsafe_allow_html=True)
                    else:
...
<body>
  ...
  <script>
    ...
    const bird = { x: 50, y: 150, width: 30, height: 30, gravity: 0.6, lift: -15, velocity: 0, 
      draw() { ctx.fillStyle = "#FFDC00"; ctx.fillRect(this.x, this.y, this.width, this.height); },
